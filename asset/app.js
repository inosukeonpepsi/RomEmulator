...
if (isMacOS) {
    if (navigator.maxTouchPoints > 2) {
        if (window.innerWidth - window.outerWidth > 30) {
            // Vision Pro?
            isMacOS = false
            isIOS = false
        } else {
...
function setVkScale() {
    var origScale = window.config.vkScale || 1;
    var scaleStr = prompt('Set virtual gamepad scale(0.1~10.0)', origScale);
    var scale = parseFloat(scaleStr);
    if (isNaN(scale) || (scale < 0.1) || (scale > 10)) {
        alert('Invalid scale.');
        return;
    }
    window.config.vkScale = scale;
}
...
async function tryLoadROM(file) {
    ...
    if (ret != 0) {
        ...
        if (ret == -1001) {
            msg += 'This file is encrypted and not supported.\n'
        }
        alert(msg)
        return;
    }
...
async function checkStorage() {
    if (navigator.storage && navigator.storage.persisted) {
        ...
        console.log('Is storage persisted?', persistent)
